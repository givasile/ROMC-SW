...
randomness of a random process is governed by the global
...
distribution, \(q_i(\thetab)\), is defined on top of each bounding
box. Each \(q_i(\thetab)\) serves a proposal distribution for
generating posterior samples \(\thetab_{ij} \sim q_i\). The samples
get an (importance) weight \(w_{ij}\) that compensate for using the
proposal distributions \(q_i(\thetab)\) instead of the prior
\(p(\thetab)\):
...
Euclidean distance, as in~\citet{Ikonomov2019}. For easier notation,
we use \(d_i(\thetab)\) to refer to
\(d(\simulator(\thetab,\ub_i), \data)\). We search for \(\theta_i^*\)
...
\end{equation}
%
The solution is accepted only if it satisfies the constraint
\(d_i(\thetab_i^*) \le \epsilon\). If not, there is no point into
building an acceptance region around \(\accregioni\), so
\(\thetab_i^*\) is discarded. There are two proposed options for
solving Equation~\ref{eq:optProb} depending on the differentiability
of \(d_i(\thetab)\). If \(d_i(\thetab)\) is differentiable any
gradient-based optimizer can be used for
Equation~\ref{eq:optProb}. The gradients
...
and any of these subsets can take any arbitrary shape. First, since
the bounding boxes are built around \(\thetab_i^*\), we focus only on
the connected subset of \(\accregioni\) that contains \(\thetab_i^*\),
which we denote as \(\accregionitheta\). Second, we want to ensure
that the bounding box \(\accregionihat\) is big enough to contain on
its interior all the volume of \(\accregionitheta\). Third, we want to
be as tight as possible to \(\accregionihat\) to ensure high
acceptance rate on the importance sampling step that will
follow. Therefore, the bounding boxes are built as follows. First, we
compute their axes \(\mathbf{v}_m\), for \(m = \{1, \ldots, D\}\)
based on the (estimated) curvature of the distance at \(\thetab_i^*\),
and, second, we apply a line-search method along each axis to determine
the size of the box, see Algorithm~\ref{alg:region_construction} for
the details. After the bounding boxes construction, a uniform
distribution \(q_i\) is defined on each bounding box, and is used as
the proposal region for importance sampling.
...
All inference methods are implemented as separate \pkg{ELFI} classes
with the two conventions:

\begin{itemize}
\item their constructor is \code{elfi.<Class name>(<output node>, *arg)};
  the initial argument is the output node of the simulator-based model
  and the rest of the arguments are parameters of the method.
\item they obtain posterior samples using \code{elfi.<Class name>.sample(*args)}
\end{itemize}
