...
algorithm, the user simply has to set \code{elfi.ROMC(<output_node>,
  parallelize = True)}.
...
function inside the \code{romc.OptimisationProblem} class; (a)
...
custom class that inherits the basic \code{OptimisatioProblem} class
...
of the \pkg{scikit-learn} package.
...
classes; (a) \code{RomcOpimisationResult} that stores the result of
the optimization step and (b) \code{NDimBoundingBox} that stores the
...
overwrite the method \code{solve_gradients()}. The method can take any
number of keyword arguments. The objective of the optimization problem
(distance \(d_i\)) can be accessed through the variable
\code{self.objective}, which is a callable. In the following snippet,
after the comment \code{state variables}, we present the state
variables that must be set by the method. The class
\code{RomcOpimisationResult(x, y, jac, hess_inv)} must be used to
...
\code{self.surrogate = custom_surrogate}, where
\code{custom_surrogate} is a \code{Callable} with signature
\code{np.ndarray (D,) -> float}, and setting
...
the \code{build_region} method. Using the objective function \(d_i\)
(\code{self.objective}) the method must estimate a list of bounding
boxes as a \code{List} with \code{NDimBoundingBox} instances and set
...
(\code{self.regions}), the method must estimate a list of local
surrogates, one for each region, as a \code{List} with callables and
