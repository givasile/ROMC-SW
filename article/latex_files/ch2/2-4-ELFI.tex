...
distribution, the ones include in the \code{elfi.Prior}
class.
...
All inference methods in \pkg{ELFI} share two rules;

\begin{itemize}
\item they follow the signature \code{elfi.<Class name>(<output node>,
    *arg)}; the initial argument is the output node of the model and the rest
  of the arguments are hyper-parameters of the method.
\item they obtain posterior samples under the API \code{<method_name>.sample()}
\end{itemize}
