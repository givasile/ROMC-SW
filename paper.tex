...
\Abstract{ Performing the inference in statistical models with
  ...
  harmonized with the rest of the ELFI framework; therefore, it is
...
modeling freedom they provide. In essence, a simulator-based model can
describe any data generating mechanism that can be written as a finite
set of algorithmic steps. This modeling freedom comes at a cost;
...
deterministic optimization processes. In their work,
~\citep{Ikonomov2019}, located some critical failure modes of OMC, and
proposed Robust OMC (ROMC) an alternative version of OMC with the
appropriate improvements.

In this paper, we present an extendible and parallelizable
...
faciliating such alterations. Finally, we have tested the accuracy and
the efficienct of our implementation on the LFI examples supported by
...
moving average (MA2) example, which is a standard model of
\pkg{ELFI}. We perform the inference using three different versions of
ROMC; (i) using a gradient-based optimizer, (ii) using the Bayesian
Optimization scheme and (iii) fitting a Neural Network as a surrogate
...
measure the execution speed-up achieved by the parallelized version of
ROMC.
...
developing the LFI method ROMC at the \proglang{ELFI} package. We paid
...
By all means, there are still open challenges for the future. Two
