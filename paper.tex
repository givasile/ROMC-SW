...
\Abstract{
  Likelihood-free inference (LFI) or Approximate Bayesian Computation (ABC) is the general framework for performing the inference in statistical models with intractable likelihood. In general, the intractability of the likelihood makes the inference problem quite challenging, therefore, many LFI methods either lack accuracy or are pretty inefficient. Robust Optimisation Monte Carlo (ROMC) is a highly-parallelizable approximate Bayesian computation (ABC) technique. In this paper, we present the implementation of ROMC in the \proglang{Python} package ELFI. Our implementation can be exploited in two directions. A scientist may use it as an out-of-the-box ABC algorithm for performing the inference in an LFI problem; we provide an easy-to-use API harmonized with the rest of the ELFI framework. Therefore, it is directly comparable with the rest of the methods included in ELFI. On the other direction, we have carefully designed ROMC in isolated algorithmic steps. Therefore, we allow the researcher to experiment with ROMC, altering or replacing parts of the algorithm with custom-created functionalities. In both scenarios, the algorithmic steps of ROMC can run in a fully-parallelizable manner, exploiting all CPU cores. We also provide some functionalities for visualization and evaluation needs. Finally, we test the robustness of our implementation on some typical LFI models that ship.
...
Simulator-based models are particularly captivating due to the modeling freedom they provide. In essence, a simulator-based model can describe any data generating mechanism that can be written as a finite set of algorithmic steps. This modeling freedom comes at a cost; performing the inference, i.e., sample or evaluate the posterior distribution, is challenging. 

Optimization Monte Carlo (OMC) proposed by [..] was a novel ABC approach approximating the posterior distribution. Afterward, Forneron and Ng provided a similar method under "reverse sampler ". The key idea is turning the stochastic data-generating mechanism into a set of fully-parallelizable deterministic optimization processes. Robust OMC (ROCM) provided an upgrade in OMC by locating and resolving some fundamental failure modes of the latter.

In this paper, we present an extendable and parallelizable implementation of ROMC at the Python package 'Engine for Likelihood-Free inference (ELFI) .' We have followed specific design principles for making the implementation extendible. Each step of the method is designed as a fully-isolated and parallelizable step; The user may easily replace parts of the technique with custom components without the backbone method collapse. Therefore, our implementation is a friendly tool for the research community to experiment with alterations of ROMC. Apart from the main functionalities, we also provide essential visualization and evaluation tools for gaining insights while fitting data. Finally, we verify the robustness of the implementation on the LFI examples supplied by the ELFI package.
...
likelihood-free inference. We then focus on the methods relevant for
this paper: first the earlier Optmisation Monte Carlo (OMC) and then
its recent robust generalisation, robust OMC, for which we provide the
software package as part of the Engine for likelihood-free inference
(ELFI), a larger inference platform that we briefly introduce at the
...
we introduce the reader to the \proglang{Python} package \pkg{Engine
  ...
  (ELFI)}. At~\ref{subsec:general_design} we explain the general
...
In this section, we illustrate the implementation performing two
experiments; In chapter~\ref{subsec:ma2} we perform the inference at
the second-order moving average (MA2) model, which is a standard model
of \pkg{ELFI}, and in chapter~\ref{subsec:exec_time} we measure the
execution speed-up achieved by the parallelization of ROMC.
