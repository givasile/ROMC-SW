...
is a \proglang{Python} package dedicated to Likelihood-Free Inference
(LFI). We implemented ROMC at \pkg{ELFI} since it provides convenient
modules for all the fundamental components of a probabilistic model
(e.g. prior, simulator, summaries etc.). Furthermore, \pkg{ELFI}
...
methods. The implementation was officially added to \pkg{ELFI} at
version \pkg{v0.8.0}.
...
sequential sampling. \pkg{ELFI} automatically considers as parameters
of interest, i.e. those we try to infer a posterior distribution,
as the ones  defined through the \code{elfi.Prior} class. Finally, the
...
All inference methods at \pkg{ELFI} share the following rules;
