...
observed that the implementation follows consistently Algorithm~\ref{alg:romc_algorithm}. The training part includes
all the steps until the computation of the proposal regions
i.e.~sampling the nuisance variables, defining the optimisation
...
unnormalised posterior (and the normalised when is possible), sampling
...
final distances or visualising the constructed bounding
...
example introduced by~\cite{Ikonomov2019}, \textbf{HP comment: what is c in Eq. \eqref{eq:1D_example_eq_2}
and in python snippet? As it is, it will not run. }
