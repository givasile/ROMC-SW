...
\code{n1} integer numbers from a discrete uniform distribution
$u_i \sim \mathcal{U}\{1, 2^{32}-1\}$. Each integer $u_i$ is the seed
used in \pkg{ELFI}'s random simulator. The seeds $u_i$ are
computationally analogous to the random variables $\vb_i$, that were
defined in the previous chapter; setting the \code{seed} eliminates all randomness of the simulator transforming it to a deterministic
